...
public class FittingViewModel extends HearingAssitstViewModel {
    ...
    public boolean fittingFreqInaudibility() {
        ...
        Log.d(TAG, "fittingFreqInaudibility: mCurrentFreq:" + mCurrentFreq + " mCurrentChannel : " + mCurrentChannel + " step: " + step);
        Log.d(TAG, "fittingFreqInaudibility: aUpper:" + aUpper + " aTemp : " + aTemp + " aLower: " + aLower);
        fittingFreq(mCurrentChannel, mCurrentFreq, db, true);
        return true;
    }
    ...
    private void enterNextFreq(float resultDB) {
        ...
        JL_Log.d(TAG, "enterNextFreq: " + resultDB + " isLast: " + isLast);
        if (!isLast) {
            //切换到下一个频率
            int channelIndex = mFreqIndex % mChannelsNum;
    ...
    private void updateChannelFreqGain(int channel, int freq, float gain, boolean isNotify) {
        ...
        Log.d("ZHM", "updateChannelFreqGain: tempArray.length : " + tempArray.length + " freqIndex: " + freqIndex);
        if (isRemoveFreq) {//降频率
