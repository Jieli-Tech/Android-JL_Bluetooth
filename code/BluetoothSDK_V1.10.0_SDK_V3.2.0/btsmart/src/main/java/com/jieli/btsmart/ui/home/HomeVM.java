...
public class HomeVM extends BtBasicVM {
    ...
    public void release() {
        super.release();
        if (LocationHelper.getInstance() != null) LocationHelper.getInstance().destroy();
    ...
    private void checkNeedShowSwitchEdrDialog(BluetoothDevice edrDevice) {
        ...
        BluetoothDevice mUseDevEdr = getBtOp().getCacheEdrDevice(getConnectedDevice());
        if (null == mUseDevEdr) return;
        BluetoothDevice mActiveDevice = getBtOp().getActivityBluetoothDevice();
    ...
    public final BTRcspEventCallback mEventCallback = new BTRcspEventCallback() {
        ...
        public void onConnection(BluetoothDevice device, int status) {
            ...
            BluetoothDevice mCacheEdr = getBtOp().getCacheEdrDevice(device);
