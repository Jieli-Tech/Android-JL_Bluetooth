...
public class LocationHelper {
    ...
    private final static int UPDATE_DEVICE_LOCATION_INFO_INTERVAL = 10 * 1000;
    private final static int MSG_UPDATE_DEVICE_LOCATION = 6984;
    private final Handler mHandler = new Handler(Looper.getMainLooper(), new Handler.Callback() {
        @Override
        public boolean handleMessage(@NonNull Message msg) {
            if (MSG_UPDATE_DEVICE_LOCATION == msg.what) {
                if (null != mAMapLocation && (mAMapLocation.getLatitude() != 0 && mAMapLocation.getLongitude() != 0) && mRCSPController.isDeviceConnected()) { //更新位置信息
    ...
    private void notifyLocationInfoChange(final AMapLocation location, final BluetoothDevice device) {
        if (location != null && mListeners.size() > 0) {
            mHandler.post(() -> {
    ...
    private void notifyRegeocodeSearched(final RegeocodeResult regeocodeResult, final int rCode) {
        if (regeocodeResult != null && mListeners.size() > 0) {
            mHandler.post(() -> {
                for (OnLocationInfoListener listener : new HashSet<>(mListeners)) {
    ...
    private void notifyGeocodeSearched(final GeocodeResult geocodeResult, final int rCode) {
        if (geocodeResult != null && mListeners.size() > 0) {
            mHandler.post(() -> {
                for (OnLocationInfoListener listener : new HashSet<>(mListeners)) {
    ...
    private final AMapLocationListener mMapLocationListener = new AMapLocationListener() {
        ...
        public void onLocationChanged(AMapLocation aMapLocation) {
            if (aMapLocation != null) {
                /*JL_Log.d(TAG, "onLocationChanged :: aMapLocation : " + aMapLocation + ",isNeedUpdateGps : " + (needUpdateGpsDev != null)
                        + ", isDevConnected : " + mBluetoothHelper.isDevConnected());*/
                mAMapLocation = aMapLocation;
                notifyLocationInfoChange(aMapLocation, null);

                if ((!mHandler.hasMessages(MSG_UPDATE_DEVICE_LOCATION) || needUpdateGpsDev != null) && mRCSPController.isDeviceConnected()) {
