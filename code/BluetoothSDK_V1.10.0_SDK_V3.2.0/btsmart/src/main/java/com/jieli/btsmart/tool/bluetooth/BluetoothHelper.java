...
public class BluetoothHelper {
    ...
    /* ------------ External Flash Op -------------------------------------------------------->>>>
     */
    public void getExternalFlashMsg(BluetoothDevice device, RcspCommandCallback callback) {
        JL_Log.d(TAG, "getDevExtFlashMsg :: device : " + BluetoothUtil.printBtDeviceInfo(device));
        sendCommand(device, CommandBuilder.buildGetExternalFlashMsgCmd(), callback);
    }

    public void writeDataToExternalFlash(BluetoothDevice device, int flag, int offset, byte[] data, RcspCommandCallback callback) {
        sendCommand(device, CommandBuilder.buildExternalFlashWriteDataCmd(flag, offset, data), callback);
    }

    public void readDataFromExternalFlash(BluetoothDevice device, int flag, int offset, int size, RcspCommandCallback callback) {
        sendCommand(device, CommandBuilder.buildExternalFlashReadDataCmd(flag, offset, size), callback);
    }

    public void startInsertFile(BluetoothDevice device, String fileName, int size, RcspCommandCallback callback) {
        sendCommand(device, CommandBuilder.buildExternalFlashInsertNewFileStartCmd(fileName, size), callback);
    }

    public void endInsertFile(BluetoothDevice device, RcspCommandCallback callback) {
        sendCommand(device, CommandBuilder.buildExternalFlashInsertNewFileEndCmd(), callback);
    }

    public void getCurrentWatchMsg(BluetoothDevice device, RcspCommandCallback callback) {
        sendCommand(device, CommandBuilder.buildExternalFlashGetCurrentWatchMsgCmd(), callback);
    }

    public void setWatchMsg(BluetoothDevice device, String filePath, RcspCommandCallback callback) {
        sendCommand(device, CommandBuilder.buildExternalFlashSetWatchCmd(filePath), callback);
    }

    public void erasureFlashData(BluetoothDevice device, int offset, int size, RcspCommandCallback callback) {
        sendCommand(device, CommandBuilder.buildExternalFlashErasureDataCmd(offset, size), callback);
    }

    public void startDeleteFlashFile(BluetoothDevice device, String filePath, RcspCommandCallback callback) {
        sendCommand(device, CommandBuilder.buildExternalFlashDeleteFileStartCmd(filePath), callback);
    }

    public void endDeleteFlashFile(BluetoothDevice device, RcspCommandCallback callback) {
        sendCommand(device, CommandBuilder.buildExternalFlashDeleteFileEndCmd(), callback);
    }

    public void extFlashWriteProtect(BluetoothDevice device, boolean isOpEnd, RcspCommandCallback callback) {
        sendCommand(device, CommandBuilder.buildExternalFlashWriteProtectCmd(isOpEnd), callback);
    }
