...
public class DevicePopDialogView extends ConstraintLayout implements ProductCacheManager.OnUpdateListener {
        ...
        switch (bleScanMessage.getAction()) {
            ...
            case ProductAction.DEVICE_ACTION_CONNECTED: {
                boolean isPhoneConnected = UIHelper.isEdrConnect(bleScanMessage.getEdrAddr()) ||
                        ...
                        || (bleScanMessage.isSupportLeAudio() && bleScanMessage.isLeAudioConnected());
    ...
    private void refreshContent(BleScanMessage bleScanMessage) {
        ...
        boolean isLeAudioConnected = bleScanMessage.isSupportLeAudio() && bleScanMessage.isLeAudioConnected();
        //是否显示电量
        boolean showQuantity = action == ProductAction.DEVICE_ACTION_CONNECTED && (isPhoneConnected || isChargingCase || isLeAudioConnected); //经典蓝牙已连接本手机或者是充电仓
...
//                JL_Log.v(tag, " -------不同设备--------");
            ...
            if (message.isSupportLeAudio() && message.isLeAudioConnected()) {
                message.setAction(ProductAction.DEVICE_ACTION_CONNECTED);
            }
        ...
        public void onDeviceCommand(BluetoothDevice device, CommandBase cmd) {
            ...
            message.setSupportLeAudio(bleScanMessage.isSupportLeAudio()).setLeAudioConnected(bleScanMessage.isLeAudioConnected());
