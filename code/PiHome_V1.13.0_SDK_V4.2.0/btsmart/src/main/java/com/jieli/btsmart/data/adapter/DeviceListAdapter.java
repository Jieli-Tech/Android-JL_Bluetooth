...
//        if (deviceName.contains("HP6699")) advInfo = null;
        if (advInfo == null || status == DEVICE_STATUS_DISCONNECTED) {
            ...
            List<ProductDesign> designList = convertFromADVInfo(history, device, advInfo);
            if (designList != null && !designList.isEmpty()) {
                for (ProductDesign design : designList) {
                    ...
                    if (design.getScene().equals(MODEL_DEVICE_LEFT_IDLE.getValue()) || design.getScene().equals(MODEL_DEVICE_LEFT_CONNECTED.getValue())) {
                        updateImageView(ivLeftDev, history, design.isGif(), design.getImageUrl(), design.getFailedRes());
                        ...
                        updateImageView(ivRightDev,  history, design.isGif(), design.getImageUrl(), design.getFailedRes());
                        ...
                        updateImageView(ivChargingBin,  history, design.isGif(), design.getImageUrl(), design.getFailedRes());
                    }
                }
            } else {
                updateProductDesign(ivLeftDev, history);
...
//            JL_Log.w(TAG, "chargingBinQuantity : " + chargingBinQuantity);
    ...
    private void updateProductDesign(ImageView imageView, HistoryBluetoothDevice item) {
        ...
        updateImageView(imageView, item, ProductUtil.isGifFile(fileUrl), fileUrl, DefaultResFactory.createBySdkType(item.getAddress(),
                item.getChipType(), item.getAdvVersion()).getLogoImg());
    }
...
//    }
    ...
    private List<ProductDesign> convertFromADVInfo(HistoryBluetoothDevice history, BluetoothDevice device, ADVInfoResponse advInfo) {
        if (advInfo == null || mPresenter == null || null == history)
            return null;
        ...
        String fileUrl;
        boolean isGif;
        DefaultResFactory.DefaultRes defaultRes = DefaultResFactory.createBySdkType(history.getAddress(), sdkType, history.getAdvVersion());
        int failRes = defaultRes.getLeftImg();
        String scene = "";
...
//        JL_Log.d(TAG, "convertFromADVInfo : " + list.size());
    ...
    private void updateImageView(ImageView imageView, HistoryBluetoothDevice history, boolean isGif, String url, int failResId) {
        if (imageView != null) {
            if (failResId <= 0 && null != history) {
                failResId = DefaultResFactory.createBySdkType(history.getAddress(), history.getChipType(), history.getAdvVersion()).getLogoImg();
            }
