...
public class ScanDeviceAdapter extends BaseQuickAdapter<ScanBtDevice, BaseViewHolder> {
    ...
    protected void convert(@NonNull BaseViewHolder helper, ScanBtDevice item) {
        ...
        int version = item.getBleScanMessage() == null ? 0 : item.getBleScanMessage().getVersion();
        helper.setImageResource(R.id.iv_scan_device_product_type, DefaultResFactory.createByDeviceType(item.getDeviceType(), version).getBlackShowIcon());
