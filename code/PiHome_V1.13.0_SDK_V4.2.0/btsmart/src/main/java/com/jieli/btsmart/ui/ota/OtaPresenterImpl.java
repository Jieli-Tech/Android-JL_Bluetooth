...
public class OtaPresenterImpl extends BluetoothBasePresenter implements IOtaContract.IOtaPresenter, DevicePopDialogFilter.IgnoreFilter {
    ...
    private void changeDeviceStatus() {
        ...
        if (null == deviceInfo || deviceInfo.isSupportDoubleBackup()) return;
        int otaWay = deviceInfo.getSingleBackupOtaWay();
        if (mRCSPController.getBtOperation().isConnectedBLEDevice(connectedDevice)) {
            if (otaWay == Constants.SINGLE_BACKUP_OTA_WAY_SPP) {
                String edrAddress = deviceInfo.getEdrAddr();
                if (CHexConver.checkBluetoothAddress(edrAddress)) {
                    mDeviceAddrManager.updateHistoryBtDeviceInfo(connectedDevice, BluetoothConstant.PROTOCOL_TYPE_SPP, edrAddress);
                }
            }
            return;
        }
        if (otaWay == Constants.SINGLE_BACKUP_OTA_WAY_BLE) {
            ...
            if (!CHexConver.checkBluetoothAddress(bleAddr)) {
