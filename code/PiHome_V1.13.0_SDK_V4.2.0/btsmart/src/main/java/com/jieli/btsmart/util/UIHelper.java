...
import com.jieli.btsmart.databinding.ItemSettingProgressBinding;
...
public class UIHelper {
    ...
    public static boolean isSoundCardType(int sdkType) {
        return JLChipFlag.JL_CHIP_FLAG_695X_SOUND_CARD == sdkType;
    }
    ...
    public static boolean isSupportTws(BluetoothDevice device) {
        if (!RCSPController.isInit()) return false;
        return isSupportTws(RCSPController.getInstance(), device);
    }

    public static boolean isSupportTws(RCSPController controller, BluetoothDevice device) {
        if (null == controller) return false;
        final DeviceInfo deviceInfo = controller.getDeviceInfo(device);
        if (null == deviceInfo) return false;
        if (deviceInfo.isSupportDevConfigure()) {
            DeviceConfiguration deviceConfiguration = controller.getDeviceConfiguration(device);
            if (deviceConfiguration instanceof TwsHeadsetConfiguration) {
                TwsHeadsetConfiguration twsHeadsetCfg = (TwsHeadsetConfiguration) deviceConfiguration;
                boolean isSupportFunctionConfiguration = twsHeadsetCfg.isSupportFunctionConfiguration();
                if (isSupportFunctionConfiguration) {
                    return twsHeadsetCfg.isSupportTws();
                }
            } else if (deviceConfiguration instanceof SoundBoxConfiguration) {
                return ((SoundBoxConfiguration) deviceConfiguration).isSupportTws();
            } else if (deviceConfiguration instanceof DongleConfiguration) {
                return ((DongleConfiguration) deviceConfiguration).isSupportTws();
            }
        }
        return isCanUseTwsCmd(deviceInfo.getSdkType());
    }
    ...
    /**
     * 判断 RecyclerView 中指定位置的 item 是否在屏幕可见区域内
     *
     * @param recyclerView      RecyclerView 实例
     * @param position          要检查的 item 在 adapter 中的位置
     * @param completelyVisible 是否要求完全可见 (true: 完全可见, false: 部分可见)
     * @return true 表示在可见区域内
     */
    public static boolean isItemVisible(@NonNull RecyclerView recyclerView, int position, boolean completelyVisible) {
        RecyclerView.ViewHolder viewHolder = recyclerView.findViewHolderForAdapterPosition(position);
        if (viewHolder == null) {
            // ViewHolder 可能尚未创建或已回收 (item 不在屏幕上)
            return false;
        }

        View view = viewHolder.itemView;
        Rect rect = new Rect();
        // 获取 view 相对于 RecyclerView 的可见矩形
        boolean isVisible = recyclerView.getChildVisibleRect(view, rect, null);
    ...
    public static void updateItemSettingsTextUI(@NonNull ItemSettingsBinding binding, String title, String value,
        ...
        binding.tvItemSettingsValue.setCompoundDrawablesRelativeWithIntrinsicBounds(0, 0, isShowArrow ? R.drawable.ic_little_right_arrow : 0, 0);
        if (isShowLine) {
    ...
    public static void updateItemSettingsProgressUI(@NonNull ItemSettingProgressBinding binding, String title, int progress,
                                                    SeekBar.OnSeekBarChangeListener listener) {
        updateItemSettingsProgressUI(binding, title, progress, CommonUtil.formatString("%d%%", progress), listener);
    }

    /**
     * 更新设置带进度布局UI
     *
     * @param binding      ItemSettingProgressBinding 布局控制器
     * @param title        String 标题
     * @param progress     int 进度
     * @param progressText String 进度文本
     * @param listener     SeekBar.OnSeekBarChangeListener 进度改变监听器
     */
    public static void updateItemSettingsProgressUI(@NonNull ItemSettingProgressBinding binding, String title, int progress, String progressText,
                                                    SeekBar.OnSeekBarChangeListener listener) {
        if (null != title) {
            binding.tvName.setText(title);
        }
        if (progress >= 0 && progress <= binding.sbProgress.getMax()) {
            binding.sbProgress.setProgress(progress);
        }
        if (null != listener) {
            binding.sbProgress.setOnSeekBarChangeListener(listener);
        }
