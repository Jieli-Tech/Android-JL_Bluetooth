...
public class HomeActivity extends NetworkActivity {
    ...
    private final Fragment[] fragments = new Fragment[]{
            ...
            DeviceListFragmentModify.newInstance()
    };
...
//            System.exit(0);
    ...
    public void onBackPressed() {
        ...
        if (AppUtil.isFastDoubleClick()) {
            //super.onBackPressed();
            JL_Log.d(TAG, "onBackPressed", "---> exit App");
            finish(); //Exit App
        } else {
...
//                if (PlayControlImpl.getInstance().isPlay())
    ...
    private int getDevDesignResId(HistoryBluetoothDevice historyBluetoothDevice, BleScanMessage bleScanMessage) {
        ...
        if(null == historyBluetoothDevice) return -1;
        int version = bleScanMessage != null ? bleScanMessage.getVersion() : historyBluetoothDevice.getAdvVersion();
        return DefaultResFactory.createBySdkType(historyBluetoothDevice.getAddress(), historyBluetoothDevice.getChipType(), version).getWhiteShowIcon();
    }
...
//    }
    ...
    private void startForegroundServiceForPop() {
        try {
            ...
            e.printStackTrace();
            JL_Log.w(TAG, "startForegroundServiceForPop", "Exception : " + e);
        }
    }
