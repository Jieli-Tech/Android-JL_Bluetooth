package com.jieli.btsmart.ui.widget.wheelview.view;
...
import com.jieli.btsmart.ui.widget.wheelview.adapter.WheelAdapter;
import com.jieli.btsmart.ui.widget.wheelview.interfaces.IPickerViewData;
import com.jieli.btsmart.ui.widget.wheelview.listener.LoopViewGestureListener;
import com.jieli.btsmart.ui.widget.wheelview.listener.OnItemSelectedListener;
import com.jieli.btsmart.ui.widget.wheelview.timer.InertiaTimerTask;
import com.jieli.btsmart.ui.widget.wheelview.timer.MessageHandler;
import com.jieli.btsmart.ui.widget.wheelview.timer.SmoothScrollTimerTask;
...
//        textSize = getResources().getDimensionPixelSize(R.dimen.pickerview_textsize);//默认大小
        ...
        textSize = (int) (16 * fontScale + 0.5f);
        ...
        if (attrs != null) {
            ...
            dividerWidth = a.getDimensionPixelSize(R.styleable.wheelview_wDividerWidth, 2);
            textSize = a.getDimensionPixelOffset(R.styleable.wheelview_wTextSize, textSize);
    ...
    public final void onItemSelected() {
        if (onItemSelectedListener != null) {
            ...
            }, 0L);//从原来的延时200ms改为0，原因：滑动已停止，不能及时回调给外层
        }
    }


    public void setDividerMore(int dividerMore) {
        this.dividerMore = dividerMore;
        invalidate();
    }
...
//        canvas.drawColor(Color.RED);
        ...
        if (dividerType == DividerType.WRAP) {//横线长度仅包裹内容
            ...
            if (TextUtils.isEmpty(label)) {//隐藏Label的情况
                startX = (measuredWidth - maxTextWidth) / 2 - dividerMore;
            } else {
                startX = (measuredWidth - maxTextWidth) / 4 - dividerMore;
            }
...
//            Log.e("sen", "startX=" + startX + "\tendX=" + endX + "\twidth=" + measuredWidth + "\tmaxTextWidth=" + maxTextWidth);

//            int x = (measuredWidth - maxTextWidth) / 2;
//            RectF r = new RectF(x, firstLineY, measuredWidth - x, secondLineY);
//            canvas.drawRect(r, paintIndicator);
...
//            canvas.drawLine(10, secondLineY - 10, measuredWidth - 10, secondLineY - 10, paintIndicator);
            ...
            float endX;
            if (TextUtils.isEmpty(label)) {//隐藏Label的情况
                startX = (measuredWidth - maxTextWidth) / 2f - dividerMore;
            } else {
                startX = (measuredWidth - maxTextWidth) / 4f - dividerMore;
            }
            if (startX <= 0) {//如果超过了WheelView的边缘
                startX = 10;
            }
        ...
        switch (mGravity) {
            case Gravity.CENTER://显示内容居中
                if (isOptions || label == null || label.equals("") || !isCenterLabel) {
                    drawCenterContentStart = measuredWidth / 2;//(int) ((measuredWidth - rect.width()) * 0.5);
...
//                    drawOutContentStart = (int) ((measuredWidth - rect.width()) * 0.5);
                    drawOutContentStart = measuredWidth / 2;
