...
//        dismissPlaySoundCtrlDialog();
    ...
    private void updateDeviceLocationUI(LatLng myPhoneLatLng) {
        ...
        for (int i = locationInfos.size() - 1; i >= 0; i--) {
            ...
            if (i == 0) {
                ...
                addDeviceMark(latLng, history, history.getName(), getLastUpdateTime(time), info.getDeviceFlag());
                if (isConnected) mConnectedDevMarker = findMark(latLng.latitude, latLng.longitude);
            } else {
                addDeviceMark(latLng, history, history.getName(), getLastUpdateTime(info.getUpdateTime()), info.getDeviceFlag());
            }
        }
    }
    ...
    private void addDeviceMark(LatLng latLng, HistoryBluetoothDevice historyDevice, String title, String desc, int deviceFlag) {
        if (null == latLng || null == historyDevice) return;
        int uid = historyDevice.getUid();
        int pid = historyDevice.getPid();
        int vid = historyDevice.getVid();
        int sdkType = historyDevice.getChipType();
        int advVersion = historyDevice.getAdvVersion();
        String iconUrl = mProductCacheManager.getProductUrl(uid, pid, vid, MODEL_LEFT_DEVICE_LOCATION.getValue());
        int failRes = DefaultResFactory.createBySdkType(historyDevice.getAddress(), sdkType, advVersion).getOnMapIcon();
...
//                    .fallback(failResId);
    ...
    private void updateDeviceUI(ImageView imageView, HistoryBluetoothDevice history, String scene) {
        ...
        DefaultResFactory.DefaultRes defaultRes = DefaultResFactory.createBySdkType(history.getAddress(), history.getChipType(), history.getAdvVersion());
        int failResId = defaultRes.getLogoImg();
        if (UIHelper.isHeadsetType(history.getChipType()) && !scene.equals(MODEL_PRODUCT_LOGO.getValue())) {
            if (scene.equals(ProductModel.MODEL_DEVICE_LEFT_IDLE.getValue()) || scene.equals(ProductModel.MODEL_DEVICE_LEFT_CONNECTED.getValue())) {
                failResId = defaultRes.getLeftImg();
                ...
                failResId =defaultRes.getRightImg();
            }
        }
