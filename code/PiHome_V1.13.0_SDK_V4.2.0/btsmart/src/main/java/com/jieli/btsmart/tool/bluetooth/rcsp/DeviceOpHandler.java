            ...
            if (status == StateCode.CONNECTION_DISCONNECT) {//判断回连设备断开
                mUIHandler.postDelayed(() -> {
                    if (!mUIHandler.hasMessages(MSG_RECONNECT_DEVICE_TIMEOUT)) { //未开始回连设备
                        JL_Log.d(DeviceOpHandler.class.getSimpleName(), "RECONNECT_DEVICE", "start reconnect");
                        //回连设备
                        BTRcspHelper.connectDeviceByMessage(mRCSPController, mReConnectDevice, null);
                        //开始回连设备超时任务
